...
module.exports = function propertySteps() {
     ...
     * Array elements can be selected to: `foo.[5]`.
     */
    this.Given(/^check property (\S+)(?: of ([\w ]+))? has type (\S+)$/,
               function(property, itemName, expected, done) {
        this._log.info('Step: check property %s of %s has type %s', property,
                       itemName, expected);
        if (itemName) {
            property = itemName + '.' + property;
        }
        var actual = typeof objTree.get(this, property);
        this._log.trace({ key: property, expected: expected, actual: actual},
                        'Comparing actual and expected types.');
        assert.deepEqual(actual, expected);
        done();
    });
    ...
    this.Given(/^I remove property (\S+)(?: of ([\w ]+))?$/,
                function (property, itemName, done) {
        this._log.info('Step: I remove property %s of %s', property, itemName);
        if (itemName) {
            property = itemName + '.' + property;
        }

        this._log.trace({ key: property }, 'Removing property.');
        objTree.remove(this, property);
        done();
    });
};
